...
shaka.util.WebVttParser = function() {
   ...
   * @returns {{cues: Array, errors: Array, time: number}}
   */
  this.parse = function(input, opt_mode) {
    ...
    if (
      ...
      err("No valid signature. (File needs to start with \"WEBVTT\".)", null)
    }
    ...
    while(lines[linePos] != "" && lines[linePos] != undefined) {
      err("No blank line after the signature.", null);
    ...
    while(lines[linePos] != undefined) {
      ...
      if(lines[linePos].indexOf("-->") == -1) {
        ...
        if(/^NOTE($|[ \t])/.test(cue.id)) { // .startsWith fails in Chrome
          ...
          while(lines[linePos] != "" && lines[linePos] != undefined) {
            if(lines[linePos].indexOf("-->") != -1)
              err("Cannot have timestamp in a comment.", null);
            linePos++
          }
          continue
        }
        ...
        if(lines[linePos] == "" || lines[linePos] == undefined) {
          err("Cue identifier cannot be standalone.", null);
          continue
        }

        if(lines[linePos].indexOf("-->") == -1) {
          parseTimings = false;
          err("Cue identifier needs to be followed by timestamp.", null)
        }
      }

      /* TIMINGS */
      alreadyCollected = false;
      var timings = shaka.util.WebVttParser.WebVTTCueTimingsAndSettingsParser(lines[linePos], err) || {};
      ...
      while(lines[linePos] != "" && lines[linePos] != undefined) {
        if(lines[linePos].indexOf("-->") != -1) {
          err("Blank line missing before cue.", null);
          alreadyCollected = true;
          break
        }
        if(cue.text != "")
      ...
      /* CUE TEXT PROCESSING */
      var cuetextparser = shaka.util.WebVttParser.WebVTTCueTextParser(cue.text, err, mode);
...
shaka.util.WebVttParser.WebVTTCueTimingsAndSettingsParser = function(line_, errorHandler) {
  ...
  /**
   * Parser for settings and time
   * @param cue
   * @param previousCueStart
   * @returns {boolean}
   */
  ...
  this.parse = function(cueStart, cueEnd) {
    ...
    while(line[pos] != undefined) {
      ...
      if(token[0] == "text") {
        ...
        var timings = shaka.util.WebVttParser.WebVTTCueTimingsAndSettingsParser(token[1], err),
