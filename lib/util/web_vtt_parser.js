...
shaka.util.WebVttParser = function() {
   ...
   */
  this.parse = function(opt_input, opt_mode) {
    //XXX need global search and replace for \0
    var NEWLINE = /\r\n|\r|\n/,
        startTime = Date.now(),
        linePos = 0,
        lines = opt_input.split(NEWLINE),
     ...
     * @param {*=} opt_col
     */
    function err(opt_message, opt_col) {
      errors.push({message: opt_message, line: linePos + 1, col: opt_col});
    };
    ...
    while (lines[linePos] != undefined) {
      ...
      if (lines[linePos].indexOf('-->') == -1) {
        ...
        /* COMMENTS
         Not part of the specification's parser as
         these would just be ignored. However,
         we want them to be conforming and not get
         "Cue identifier cannot be standalone".
         */
      ...
      alreadyCollected = false;
      var timings =
          new shaka.util.WebVttParser.
              WebVTTCueTimingsAndSettingsParser(lines[linePos], err);
      ...
      var cuetextparser =
          new shaka.util.WebVttParser.
              WebVTTCueTextParser(cue.text, err, opt_mode);
 ...
 * @constructor
 */
shaka.util.WebVttParser.
    WebVTTCueTimingsAndSettingsParser = function(line_, errorHandler) {
  var SPACE = /[\u0020\t\f]/,
      NOSPACE = /[^\u0020\t\f]/,
      line = line_,
      pos = 0,
  ...
  function timestamp() {
    ...
    return parseInt(val1, 10) * 60 * 60 +
        parseInt(val2, 10) * 60 +
        parseInt(val3, 10) + parseInt(val4, 10) / 1000;
  }
    ...
    for (var i = 0; i < settings.length; i++) {
      ...
      if (setting == 'vertical') { // writing direction
        ...
        if (alignValues.indexOf(value) == -1) {
          err('Alignment can only be set to one of ' +
              alignValues.join(', ') + '.');
          continue;
        }
        cue.alignment = value;
  ...
  this.parse = function(cue, previousCueStart) {
    ...
    if (cue.startTime < previousCueStart) {
      err('Start timestamp is not greater ' +
          'than or equal to start timestamp of previous cue.');
    }
 ...
 * @constructor
 */
shaka.util.WebVttParser.WebVTTCueTextParser =
    function(line_, errorHandler, mode) {
  var line = line_,
      pos = 0,
      err = function(message) {
        if (mode == 'metadata')
  ...
  this.parse = function(cueStart, cueEnd) {
    ...
    function attach(token) {
      current.children.push({type: 'object',
        name: token[1], classes: token[2], children: [], parent: current});
    ...
    while (line[pos] != undefined) {
      ...
      if (token[0] == 'text') {
        ...
        var timings = new shaka.util.WebVttParser
            .WebVTTCueTimingsAndSettingsParser(token[1], err),
        ...
        if (timestamp != undefined) {
          ...
          current.children.push({type: 'timestamp',
            value: timestamp, parent: current});
