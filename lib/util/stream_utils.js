...
shaka.util.StreamUtils.meetsRestrictions = function(
  ...
  // shaka.log.info('meetsRestrictions...');
  // shaka.log.info(variant);
  //
  // shaka.log.info('Excuded role..' + restrictions.excludedRole);
  // // This fix will not work for < IE 9. Need better solution.
  // // But checks if we added filterCondition on role to exclude
  // if (audio.roles &&
  //     audio.roles.indexOf(restrictions.excludedRole) === -1) {
  //   shaka.log.debug('restrictions.excludedRole ' +
  // restrictions.excludedRole);
  //   return false;
  // }
...
shaka.util.StreamUtils.getVariantTracks =
  ...
  var tracks = variants.map(function(variant) {
    ...
    var language = variant.audio ? variant.audio.language : variant.language;
