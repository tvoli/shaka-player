...
/**
 * This method modifies the variants so they include the audio language set within
 * the audio track within the variant, if it exists. MAGINE ADDED
 *
 * @param {!Array.<!shakaExtern.Variant>} variants
 * @return {!Array.<!shakaExtern.Variant>} variants
 */
shaka.util.StreamUtils.modifyVariantsWithAudioLanguage =
    function(variants){
...
shaka.util.StreamUtils.getVariantTracks =
  ...
  // Variant fix
  variants = StreamUtils.modifyVariantsWithAudioLanguage(variants);
