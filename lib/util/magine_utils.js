 ...
 * @return {string}
 */
shaka.util.MagineUtils.returnAccessibLang = function(language) {
 ...
 */
shaka.util.MagineUtils.hasAccessibTag = function(elem) {
 ...
 */
shaka.util.MagineUtils.modifyIfAccessibTag =
    function(roles, elem) {
  var XmlUtils = shaka.util.XmlUtils,
      accessbilityElems = XmlUtils.findChildren(elem, 'Accessibility'),
      modifiedRoles = roles,
  ...
  if (accessbilityElems.length > 0) {
    ...
    containsMainRole = MagineUtils.stringExistsWithinArray(roles, 'main');
    if (containsMainRole) {
      // Remove the main element at index
      modifiedRoles = MagineUtils.arrayItemRemover(roles, 'main');
    }
    ...
    containsNoMainRole =
        !Boolean(MagineUtils.stringExistsWithinArray(roles, 'main'));
    containsNoCommentaryRole =
        !Boolean(MagineUtils.stringExistsWithinArray(roles, 'commentary'));
 ...
 */
shaka.util.MagineUtils.stringExistsWithinArray =
    function(arr, string) {
 ...
 */
shaka.util.MagineUtils.arrayItemRemover =
    function(arr, string) {
  var newArray = [];
