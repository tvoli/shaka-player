    ...
    try {
      ...
      var objLastInserted = this.inserted_
        ...
        .filter(function(segmentReference) {
            return segmentReference.url.toString()
              .indexOf(objTrack.label) !== -1;
          })
      ...
      if (!objLastInserted) {
        shaka.log.debug('Cannot figure out last added text track,',
            'possible overrun');
      }

      numDifference = objLastInserted &&
          objLastInserted.endTime - timestamp || 0;

      shaka.log.debug('Text Tracks Manager: bufferedAheadOf',
          numDifference, objLastInserted);
    } catch (e) {
