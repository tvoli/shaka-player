...
shaka.media.StreamingEngine.prototype.fetchAndAppend_ = function(
  ...
  Promise.all([initSourceBuffer, fetchSegment]).then(function(results) {
    ...
    if (results[1] && results[1].sar) {
      var offset = availabilityStart +
          timeline.getPresentationStartTime();
      window.dispatchEvent(new CustomEvent('aspectratioupdate',
          { 'detail': { 'sar': results[1].sar, 'offset': offset }}));
    }
...
shaka.media.StreamingEngine.prototype.fetch_ = function(reference) {
  ...
  return p.then(function(response) {
    for (var header in response.headers) {
      if (header.toString().trim() === 'x-amz-meta-sar') {
        response.data.sar = response.headers[header];
      }
    }
