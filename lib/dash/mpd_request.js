...
shaka.dash.MpdRequest = function(url, opt_requestTimeout) {
  ...
  /** @protected {!shaka.util.FailoverUri} */
  this.url_ = url;
...
shaka.dash.MpdRequest.prototype.send = function() {
  var url = this.url_;
  var mpdPostProcessor = this.mpdPostProcessor_;
  return url.fetch(this.parameters_).then(
      /** @param {!ArrayBuffer|string} data */
      function(data) {
        if (data && mpdPostProcessor) {
          data = mpdPostProcessor(data);
        }
        var mpd =
            shaka.dash.mpd.parseMpd(/** @type {string} */ (data), url.urls);
        if (mpd) {
...
/**
 * Manipulates the Mpd after the request is done
 * @param {Function} mpdPostProcessor The post processor
 * @return {shaka.dash.MpdRequest}
 */
shaka.dash.MpdRequest.prototype.mpdPostProcessor = function(mpdPostProcessor) {
