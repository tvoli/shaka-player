...
goog.require('shaka.util.MagineUtils');
...
shaka.dash.DashParser.prototype.parseAdaptationSet_ = function(context, elem) {
  ...
  roleValues = shaka.util.MagineUtils.modifyIfAccessibTag(roleValues, elem);
  ...
  // Magine FIX for descriptive audio language
  var hasAccessibilityTag = shaka.util.MagineUtils.hasAccessibTag(elem);
  if (hasAccessibilityTag) {
    language = shaka.util.MagineUtils.returnAccessibLang(language);
    // Verifying that even though we changed the language
    // it passes the shaka-player normalization
    var languageNormalized = shaka.util.LanguageUtils.normalize(language);
    shaka.log.info('Normalized language -  ' + languageNormalized);
  }
...
shaka.dash.DashParser.prototype.parseRepresentation_ = function(
  ...
  // Added Magine fix for accessibility language
  var hasAccessibility = shaka.util.MagineUtils.hasAccessibTag(node);

  // Lets call our util method to add role 'commentary' if
  // there are accessibility tags on adaptionSet. Added 2017-08-17
  roles = shaka.util.MagineUtils.modifyIfAccessibTag(roles, node);

  // This might not be called, but still added this
  // in case of language set on different set, adaption/representation
  if (hasAccessibility) {
    language = shaka.util.MagineUtils.returnAccessibLang(language);
  }
