...
shaka.dash.DashParser.prototype.parseManifest_ =
  ...
  if (xml) {
    ...
    shaka.log.info('This is the mpd object..');
    shaka.log.info(mpd);
  ...
  // Added Magine fix
  var accessibilityTags = XmlUtils.findChildren(mpd, 'Accessibility');
  shaka.log.info('Accessbility..?');
  shaka.log.info(accessibilityTags);
...
shaka.dash.DashParser.prototype.parsePeriod_ = function(
  ...
  shaka.log.info('OKOKOKOKO');
  shaka.log.info(adaptationSets);
  ...
  shaka.log.info('Audio Sets');
  shaka.log.info(audioSets);
  shaka.log.info('Video Sets');
  shaka.log.info(videoSets);
 ...
 * @private
 */
shaka.dash.DashParser.prototype.modifyIfAccessibTag_ = function(roles, elem) {
  var XmlUtils = shaka.util.XmlUtils;
  var accessbilityElems = XmlUtils.findChildren(elem, 'Accessibility');
  var modifiedRoles = roles;
...
shaka.dash.DashParser.prototype.parseAdaptationSet_ = function(context, elem) {
  ...
  // Lets call our util method to add role 'commentary' if
  // there are accessibility tags on adaptionSet
  roleValues = this.modifyIfAccessibTag_(roleValues, elem);
...
shaka.dash.DashParser.prototype.parseRepresentation_ = function(
  ...
  // Lets call our util method to add role 'commentary' if
  // there are accessibility tags on adaptionSet
  roles = this.modifyIfAccessibTag_(roles, node);

  // var accesibilityElems = XmlUtils.findChildren(node, 'Accessibility');
  // var accessbilityText = '';

  // shaka.log.info('Accessbility for the representation..');
  // shaka.log.info(accesibilityElems);

  // if (accesibilityElems.length > 0) {
  //   roles.push('commentary');
  //   accessbilityText = '- YES!';
  // }

  // roles.push('commentary');
  // Lets add 'commentary' to the roles array here..? :D
  ...
  shaka.log.info('TestObjet - ' + testObject.roles.length);
  shaka.log.info(testObject);
