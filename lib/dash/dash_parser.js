 ...
 * @private
 */
shaka.dash.DashParser.prototype.hasAccessibTag_ = function(elem) {
  var XmlUtils = shaka.util.XmlUtils,
 ...
 */
shaka.dash.DashParser.prototype.stringExistsWithinArray =
    function(arr, string) {
  var stringExists = arr.indexOf(string) === -1 ? false : true;
  return stringExists;
};
 ...
 */
shaka.dash.DashParser.prototype.arrayItemRemover = function(arr, string) {
  var newArray = [];
...
shaka.dash.DashParser.prototype.parseRepresentation_ = function(
  ...
  // ADDED STUFF HERE.. COULD REMOVE LATER..
  var hasAccessibility = roles.indexOf('commentary') === -1 ? false : true;
  if (hasAccessibility) {
    language = language + '-accessbility';
  }
