...
shaka.dash.DashParser.prototype.modifyIfAccessibTag_ = function(roles, elem) {
  ...
  if (accessbilityElems.length > 0) {
    ...
    var containsMainRole = roles.indexOf('main') === -1 ? false : true;
    if (containsMainRole) {
      mainRoleIndex = roles.indexOf('main');
      // Remove the main element at index
      modifiedRoles = modifiedRoles.splice(mainRoleIndex, 1);
    }
    ...
    var containsNoMainRole =
        roles.indexOf('main') === -1 ? true : false;
    var containsCommentaryRole =
...
shaka.dash.DashParser.prototype.parseRepresentation_ = function(
  ...
  shaka.log.info('Printing from parseRepresentation...');
